...
describe('PatientRegistrationForm', () => {
  ...
  test('defaults preferred language to Tamil', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/Email Address/i), { target: { value: 'testpref@example.com' } });
    ...
    fireEvent.change(screen.getByLabelText(/Street\/House No./i), { target: { value: '456 Test St' } });
    fireEvent.change(screen.getByLabelText(/City/i), { target: { value: 'Chennai' } });
    fireEvent.change(screen.getByLabelText(/State/i), { target: { value: 'Tamil Nadu' } });
  ...
  test('auto-fills emergency contact details when relationship is Self', async () => {
    ...
    // Step 1: Personal Details
    fireEvent.change(screen.getByLabelText(/primary phone number/i), { target: { value: testPatientPhone } });
    ...
    fireEvent.change(screen.getByLabelText(/Street\/House No./i), { target: { value: '789 Self St' } });
    fireEvent.change(screen.getByLabelText(/City/i), { target: { value: 'Chennai' } });
    fireEvent.change(screen.getByLabelText(/State/i), { target: { value: 'Tamil Nadu' } });
    ...
    await waitFor(() => {
      const emergencyNameInput = screen.getByLabelText(/^Full Name$/i, { selector: '#emergencyName' });
    ...
    const emergencyPhoneInput = screen.getByLabelText(/^Phone Number$/i, { selector: '#emergencyPhone' });
